...
//		String yyyyMM = new SimpleDateFormat("yyyyMM").format(date);
    ...
    public static String formatDateBetweenString(Date beginDate, Date endDate) {
        ...
        if (StringUtil.isNoneBlank(begin, end)) {
            return begin + " ~ " + end;
        }
        return null;
    }
     ...
     * 解析日期范围字符串为日期对象
     *
     * @param dateString 2019-01-01 ~ 2019-01-31
     * @return new Date[]{2019-01-01, 2019-01-31}
     * @author ThinkGem
     */
    public static Date[] parseDateBetweenString(String dateString) {
        ...
        if (StringUtil.isNotBlank(dateString)) {
            String[] ss = StringUtil.split(dateString, "~");
            if (ss != null && ss.length == 2) {
                String begin = StringUtil.trim(ss[0]);
                String end = StringUtil.trim(ss[1]);
                if (StringUtil.isNoneBlank(begin, end)) {
