package com.iyysoft.msdp.common.security.util;
...
public class Pkcs7Encoder {
    ...
    public static byte[] decryptOfDiyIV(byte[] encryptedData, byte[] keyBytes, byte[] ivs) {
        ...
        init(keyBytes);
        try {
            //cipher.init(Cipher.DECRYPT_MODE, key, new AlgorithmParameters (ivs));
            if (key == null){
                return null;
            }
    ...
    public static String decryptAES(String data, String key, String iv) {
        byte[] sessionKeyBy = key.getBytes();
        byte[] encryptedDataBy = Base64.decode(data.getBytes(StandardCharsets.UTF_8));
    ...
    public static String encryptAES(String data, String key, String iv) {
        ...
        byte[] encryptedDataBy = Base64.decode(data.getBytes(StandardCharsets.UTF_8));
        byte[] ivBy = iv.getBytes();
        byte[] esc = Pkcs7Encoder.encryptOfDiyIV(encryptedDataBy, sessionKeyBy, ivBy);
        return new String(esc, StandardCharsets.UTF_8);
